...
class InstallerMessagingController extends GetxController {
  ...
  Future<void> _initializeChat() async {
    ...
    try {
      ...
      if (resolvedUserId == null || resolvedUserId.isEmpty) {
        AppLoggerHelper.warning('Installer chat init failed: missing user id');
        return;
      }
      ...
      AppLoggerHelper.error('Installer chat init failed', error);
    } finally {
  ...
  Future<bool> sendSocketMessage({
    ...
    if (!isSocketConnected.value || _socket == null) {
      AppLoggerHelper.warning(
        'Installer chat send skipped: socket disconnected',
      );
      Get.snackbar(
        'Connection error',
    ...
    try {
      ...
      AppLoggerHelper.error('Installer message send failed', error);
      Get.snackbar(
        'Send failed',
  ...
  Future<void> editMessage({
    ...
    if (!_isSocketReadyForAction('Installer message edit')) return;

    _sendSocketPayload(
      {'action': 'edit', 'message_id': id, 'new_text': text},
      actionName: 'edit',
    );
  }
  ...
  Future<void> deleteMessage({required String messageId}) async {
    ...
    if (!_isSocketReadyForAction('Installer message delete')) return;

    _sendSocketPayload(
      {'action': 'delete', 'message_id': id},
      actionName: 'delete',
    );
  }
  ...
  Future<void> reactToMessage({
    ...
    if (!_isSocketReadyForAction('Installer message react')) return;
    ...
    _sendSocketPayload(
      {'action': 'react', 'message_id': id, 'reaction': emoji},
      actionName: 'react',
    );
  }
  ...
  Future<void> removeReaction({
    ...
    final emoji = reaction.trim();
    if (id.isEmpty || emoji.isEmpty) return;
    if (!_isSocketReadyForAction('Installer message remove react')) return;
  ...
  Future<void> _connectSocket({required String userId}) async {
    ...
    _socketSubscription = _socket!.stream.listen(
      ...
      onError: (Object error) {
        ...
        AppLoggerHelper.error('Installer websocket error', error);
      },
      onDone: () {
        isSocketConnected.value = false;
        AppLoggerHelper.warning('Installer websocket closed');
      },
      cancelOnError: true,
    );
    isSocketConnected.value = true;
    AppLoggerHelper.info('Installer websocket connected: $wsUrl');
  }
  ...
  void _handleSocketEvent(dynamic rawEvent) {
    ...
    AppLoggerHelper.debug(
      'Installer WS IN (raw, ${payloadText.length} chars): '
    ...
    try {
      ...
      AppLoggerHelper.debug(
        'Installer WS IN (decoded): ${_summarizePayloadForLog(payload)}',
      );
      ...
      AppLoggerHelper.error('Installer socket payload parse failed', error);
    }
  }
  ...
  bool _isMessageForCurrentConversation(
    ...
    if (fromType != null && fromType.isNotEmpty) {
      ...
      if (normalizedFromType != _peerTypePlural &&
        ...
        AppLoggerHelper.debug(
          'Installer chat accepted message with unexpected from_type=$fromType',
        );
      }
    }
  ...
  Future<void> _loadHistory({
    ...
    final mapped = history.map((ChatHistoryMessage historyMessage) {
      final id = historyMessage.messageId?.trim() ?? '';
      return Message(
        id: id,
  ...
  Future<void> _closeSocket() async {
    AppLoggerHelper.info('Installer websocket closing');
  ...
  Future<void> _resolveCurrentUserName(String authorization) async {
    try {
      ...
      AppLoggerHelper.warning('Installer chat name resolve skipped: $error');
    }
  }

  String get _selfTypePlural => 'installers';
  String get _peerTypePlural {
    final raw = conversation.userType;
    if (raw.trim().isEmpty) return 'customers';
  ...
  void _sendSocketPayload(
    ...
    AppLoggerHelper.debug(
      'Installer WS OUT [$actionName]: ${_summarizePayloadForLog(payload)}',
    );
  ...
  bool _handleNonMessagingSocketPayload(Map<String, dynamic> payload) {
    final errorText =
        _normalizeString(payload['error']) ?? _normalizeString(payload['detail']);
    if (errorText != null) {
      AppLoggerHelper.warning(
        'Installer WS ERROR RESPONSE: $errorText '
    ...
    AppLoggerHelper.info(
      'Installer WS non-messaging payload: ${_summarizePayloadForLog(payload)}',
    );
    return true;
  }
  ...
  void _handleControlPayload(Map<String, dynamic> payload) {
    ...
    if (action == null) {
      AppLoggerHelper.info(
        'Installer WS control payload missing action: '
    ...
    AppLoggerHelper.info(
      'Installer WS unsupported control action=$action '
      'payload=${_summarizePayloadForLog(payload)}',
    );
  }
  ...
  void _handleStatusAckPayload({
    ...
    if (errorStatuses.contains(status)) {
      ...
      AppLoggerHelper.warning(
        'Installer WS status error: $errorText '
        'payload=${_summarizePayloadForLog(payload)}',
      );
      Get.snackbar(
        'Message error',
    ...
    if (editStatuses.contains(status)) {
      AppLoggerHelper.info(
        'Installer WS edit status: ${_summarizePayloadForLog(payload)}',
      );
    } else if (deleteStatuses.contains(status)) {
      AppLoggerHelper.info(
        'Installer WS delete status: ${_summarizePayloadForLog(payload)}',
      );
    } else {
      AppLoggerHelper.info(
        'Installer WS status payload: ${_summarizePayloadForLog(payload)}',
      );
    }
  ...
  Future<void> _syncHistoryAfterStatusAck(String status) async {
    ...
    try {
      ...
      AppLoggerHelper.warning(
        'Installer history sync skipped after status=$status: $error',
      );
    } finally {
